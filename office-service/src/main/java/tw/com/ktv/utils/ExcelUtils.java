...
import tw.com.ktv.utils.excel.CellInfo;
import tw.com.ktv.utils.excel.HSSFCellStyleDto;
import tw.com.ktv.utils.excel.HSSFFontDto;
import tw.com.ktv.utils.excel.HSSFFontFunc;
import tw.com.ktv.utils.excel.SheetInfo;
 ...
 * @since 2015-02-13
 * @version 1.0
 * @version 2.0 2015-03-22 修改參數傳遞方式
 * @version 2.1 2015-03-30 1.增加setCellValue的參數纇, 2.整理程式
 * @version 2.2 2015-06-23 增加枚舉類調用
 * @version 2.2 2015-06-26 枚舉類調用方式有兩種
 * @version 2.3 2015-07-10 解決：The maximum number of cell styles was exceeded. You can define up to
 *          4000 styles in a .xls workbook
 * @version 2.4 2015-08-11 HSSFCellStyle 與 HSSFSheet 獨立寫 (可自定義)
 */
@Slf4j
public class ExcelUtils {
  ...
  /**
   * 透過dto的註釋 ,生成excel表 (一次處理一個sheet) 1. 若相同dto使用不同sheetName, dto 需加入private String sheetName
   * (List第一筆自行變更值)
   * 
   * @param dataListArray <dto> ex:fc.wpf.rest.report.domain.FCWithdrawActTimeDto
   * @return HSSFWorkbook
   */
  public static HSSFWorkbook creatExcelDataFromDto(List<?>... dataListArray) {
    ...
    for (List<?> dataList : dataListArray) {
      ...
      HSSFSheet sheet = ExcelUtils.getHSSFSheet(workbook, titleDto);

      Map<String, CellInfo> tmpMap = new HashMap<String, CellInfo>();

      // title
      ExcelUtils.setTitleRow(workbook, sheet, titleDto, tmpMap);

      // detail
      ExcelUtils.setDetailRow(workbook, sheet, dataList, tmpMap);
    }
   ...
   * @param tmpMap
   */
  private static void setTitleRow(HSSFWorkbook workbook, HSSFSheet sheet, Object titleDto,
    ...
    for (Object dto : dataList) {
      ...
      for (Method method : methods) {
        ...
        if (info != null) { // Getter
          ...
          ExcelUtils.setCellValue(info, cell, dto, method);

          // fontStyle
          HSSFFontDto fontDto = new HSSFFontDto(defaultFont);
          ExcelUtils.setFontStyle(info, fontDto, dto, method);

          // curStyle
          HSSFCellStyleDto curStyleDto = new HSSFCellStyleDto(defaultStyle);
          ExcelUtils.setCellStyle(info, curStyleDto);
          HSSFCellStyle curStyle = fontFunc.getCellStyle(workbook, curStyleDto, fontDto);
          cell.setCellStyle(curStyle);
        }
      }
  ...
  private static void setCellValue(CellInfo info, HSSFCell cell, Object dto, Method method) {
    try {
      ...
      if (obj instanceof String) {
        ...
        Date date = (Date) obj;
        SimpleDateFormat sdf = new SimpleDateFormat(info.dateStyle().getFormat());
   ...
   * @param method
   */
  private static void setFontStyle(CellInfo info, HSSFFontDto styleDto, Object tmpDto,
      Method tmpMethod) {
    ...
    if (!CellInfo.class.equals(info.formatFontStyle())) {
      Method method;
      try {
        method = info.formatFontStyle().getMethod("formatFontStyle",
